...
po.image = function() {
  ...
  function load(tile) {
    ...
    if (typeof url == "function") {
      ...
      // ABL Hack to get fresh tile if served from our tile server
      if (url(tile).indexOf(window.location.origin) == 0) {
        tileUrl = tileUrl + '?' + Math.random();
      }
...
po.stylist = function() {
  ...
  function stylist(e) {
    ...
    for (i = 0; i < ne; ++i) {
      ...
      for (j = 0; j < ns; ++j) {
        v = (x = styles[j]).value;
        if (typeof v === "function") v = v.call(null, d, e);
        v == null
